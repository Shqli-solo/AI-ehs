...
> **状态**: APPROVED  
> **模式**: 渐进式重构  
> **执行策略**: Wave 1+2 并行 → Wave 3 → Wave 4 → Wave 5  
> **TOGAF ADM 阶段**: A-H 完整覆盖  
> **DDD**: 战略设计 + 战术设计

## 1.1 确认决策 (2026-04-17)

- **范围**: 完整执行全部 5 个 Wave，不裁剪
- **模式**: 渐进式重构，在现有 apps/ 基础上逐步完善
- **执行策略**: Wave 1（数据+前端+Docker）与 Wave 2（微调训练）并行
- **前端 UI 库**: Shadcn/UI（已安装）
- **前端路由**: Next.js App Router（已迁移完成）  
